...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h2 class="text-xl font-bold text-gray-800">Liste des Médias</h2>
        <a href="{{ route('admin.medias.create') }}" class="inline-flex items-center px-4 py-2 bg-orange-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-orange-700 active:bg-orange-800 focus:outline-none focus:border-orange-900 focus:ring ring-orange-300 disabled:opacity-25 transition ease-in-out duration-150">
    ...
    <div class="overflow-x-auto">
        <table id="mediasTable" class="w-full text-sm text-left text-gray-500">
            <thead class="text-xs text-white uppercase bg-orange-600">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Contenu Associé</th>
                    <th scope="col" class="px-6 py-3">Date d'upload</th>
        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    // Si c'est une image, afficher une miniature (optionnel)
                    // Pour l'instant on affiche juste le texte/chemin
                    return (data && data.length > 30) ? data.substring(0, 30) + '...' : data;
                }
            },
            { data: 'type_media', name: 'type_media' },
